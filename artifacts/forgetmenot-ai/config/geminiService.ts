 ...
 */
// ✅ Defined explicit input contract interface parameters
...
export const fetchGeminiSignalAnalysis = async (input: GeminiAnalysisInput): Promise<any> => {
  if (!GEMINI_API_KEY) {
    throw new Error("Missing EXPO_PUBLIC_GEMINI_API_KEY environment parameter token.");
  }

  // Set up structural context configuration preferences defaults
  const safePrompt = input.userPrompt || "";
  const safeCategory = (input.selectedCategory || 'practical').toLowerCase();

  const systemicSystemInstruction = `
    You are the omission-risk analysis engine for ForgetMeNot AI™.

    Analyze the user's input context (which may include text notes, an uploaded image file, or both) and identify ONE primary thing they are most likely to forget.
  ...
  try {
    console.log(`🔮 Sending telemetry bytes out to Google Gemini REST Gateway API engine... Category Scope: [${safeCategory}]`);

    // 📦 Assemble contents payload elements dynamically based on user upload parameters
    const dynamicPartsArray: any[] = [];

    // 📸 Scenario A: If user attached a camera or library picture, pack the image parts structure
    if (input.imagePayload && input.imagePayload.base64) {
      dynamicPartsArray.push({
        inlineData: {
          mimeType: input.imagePayload.mimeType || "image/jpeg",
          data: input.imagePayload.base64
        }
      });
      console.log("📷 Image buffer injected successfully into API gateway package stream.");
    }

    // 📝 Scenario B: Construct descriptive contextual directives passing user strings/notes
    const coreContextTextDirective = safePrompt.trim()
      ? `Current User Context Input Notes: "${safePrompt}"\nSelected Category Guideline Preference: ${safeCategory}`
      : `Analyze this image attachment framework context directly.\nSelected Category Guideline Preference: ${safeCategory}`;

    dynamicPartsArray.push({ text: coreContextTextDirective });
        ...
        generationConfig: {
          ...
          responseSchema: {
            ...
            properties: {
              ...
              explanation: {
                type: "string",
                description: "A concise paragraph detailing why this travel or schedule transition makes them vulnerable. Use natural, plain English sentences."
              },
              preventiveAction: {
                type: "string",
