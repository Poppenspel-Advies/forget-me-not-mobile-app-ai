...
export function MemoryScreen({ onBack, captured = [] }: MemoryScreenProps) {
  ...
  const [loading, setLoading] = useState(true);
     ...
     /*  // 🌟 UPGRADED PURGE DELETE: Executes safe deletion step
        const handleDeleteSignal = async (documentId: string) => {
          try {
            console.log(`🗑️ Removing record from cloud bucket pointer: [${documentId}]`);
            const documentReference = doc(db, "analyses", documentId);
            await deleteDoc(documentReference);
            setConfirmDeleteId(null); // Reset confirmation state
            triggerToast("Signal map context deleted successfully.");
          } catch (error) {
            console.error('💥 Crash running cloud item deletion trace:', error);
          }
        };
 */
      /* // 🌟 ADDED: Opens the editing drawer with data from the tapped item pre-filled
      const handleOpenEditDrawer = (item: CapturedItem) => {
        setSelectedEditId(item.id);
        setEditTitleText(item.title);
        setEditDetailText(item.detail);
      ...
      snapshot.forEach((doc) => {
        const data = doc.data();
         // 🌟 EXTRACT CATEGORY TAG FROM ROOT DATA PREFERENCES:
          const computedTag = (data.tag || "THINGS").toUpperCase();
        ...
        liveRecords.push({
          ...
          tag: computedTag,
          color: computedColor,
          capture_image_url: data.capture_image_url || null,
          voice_memo_url: data.voice_memo_url || null
        });
      });

      console.log(`📊 LIVE STREAMING: Hooked ${liveRecords.length} records successfully.`);
